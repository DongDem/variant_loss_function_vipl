...
def build_network(input_images, labels, ratio,keep_prob):
    ...
    return logits, features, total_loss, accuracy, softmax_loss


logits, features, total_loss, accuracy, softmax_loss = build_network(input_images, labels,ratio=LAMBDA, keep_prob=keep_prob)
...
while step <= 8000:
    ...
    _, summary_str, train_acc, printed_total_loss, printed_softmax_loss = sess.run([train_op, summary_op, accuracy, total_loss , softmax_loss],
...
save_path = saver.save(sess, "../model_alexnet_MMI/MMI_with_softmax_loss_alexnet_fold10.ckpt")
